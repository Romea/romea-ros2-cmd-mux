...
public :
  ...
  TestableCmdMux(const rclcpp::NodeOptions & options):
    CmdMux(options)
  {
...
public :
  ...
  void SetUp()
  {
    ...
    cmd_mux_subscription_client = std::make_unique<romea::CmdMuxSubscriptionClient>(cmd_mux->get_node());
    cmd_mux_unsubscription_client = std::make_unique<romea::CmdMuxUnsubscriptionClient>(cmd_mux->get_node());
  }
