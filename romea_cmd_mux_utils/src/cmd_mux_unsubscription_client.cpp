...
void throw_fail_to_send_ununsubscription_request(const std::string & service_name)
{
  ...
  msg<< ", cannot unregister from "<< cmd_mux_name;
  throw std::runtime_error(msg.str());
}
...
//-----------------------------------------------------------------------------
CmdMuxUnsubscriptionClient::Result CmdMuxUnsubscriptionClient::unsubscribe_(const std::string &topic)
{
...
void CmdMuxUnsubscriptionClient::unsubscribe(const std::string &topic_name)
{
  ...
  case Result::FAIL_TO_CALL_SERVICE:
    throw_fail_to_call_ununsubscription_service(service_name);
    break;
  case Result::FAIL_TO_SEND_REQUEST:
    throw_fail_to_send_ununsubscription_request(service_name);
    break;
  }
